...
router.post("/product/", upload.single("image"), (req, res) => {
  ...
  const checkImage = image ? console.log("ada") : console.log("ga ada"); //check ada image karna menggunakan ubuntu buth permit
  if (image) {
    const target = path.join(__dirname, "uploads", image.originalname); // join(direktory, target, urldikirim)
    fs.renameSync(image.path, target);
    // res.sendFile(target); //send response dalam bentuk file propertiesobject) static file
  } // convert file to original
  res.json({
    name,
    price,
    stocks,
    status,
    image,
  }); //send response dalam json yang didapat dari objects body
});
/**
 * ! Flownya
